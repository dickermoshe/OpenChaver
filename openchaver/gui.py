...
def configure_gui():
    ...
    window = sg.Window('OpenChaver Configuration', layout,finalize=True,size=(400,200),element_justification='center')
    while True:
        ...
        if event == sg.WIN_CLOSED or event == 'Cancel': # if user closes window or clicks cancel
            ...
            try:
                r = requests.post(API_BASE_URL+f"devices/register_device/", data=data, verify=False)
            except:
...
def uninstall_gui(device_id):
    ...
    window = sg.Window('OpenChaver Uninstall', layout,finalize=True,size=(400,200),element_justification='center')
    while True:
        event, values = window.read()
        if event == sg.WIN_CLOSED or event == 'Cancel': # if user closes window or clicks cancel
            break
            ...
            try:
                r = requests.post(API_BASE_URL+f"devices/verify_uninstall_code/", data=data, verify=False)
            except:
                sg.popup("Error connecting to server")
                continue
            if r.status_code == 200:
